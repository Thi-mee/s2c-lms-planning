...
**Current concrete seam (2026-09-18):** Authoring and Enrollment depend on Identity's public `IIdentityAccess`/`IdentityWork` contract. Their writes open one PostgreSQL transaction, acquire the same organization row guard as role changes/deactivation, then recheck current authority. Each module uses the exposed local connection/transaction only for its own schema; account eligibility and audit persistence remain Identity operations. Enrollment resolves a published Course through Authoring's public `IAuthoringAccess`, while Identity invokes Enrollment's registered `IAccountAccessRemovalGuard` before a role removal or deactivation that could invalidate cohort staffing. Identity and Authoring do not reference Enrollment. Module implementations remain internal, and architecture tests prohibit reverse/host dependencies. This coarse guard trades write concurrency for a simple, proven boundary in the initial deployment. Revisit it from measured contention, not hypothetical scale. `Variable.Database` contains only the migration manifest/runner shared by the implemented modules; the host composes their immutable SQL resources. See the concrete [authoring](../04-api-design/course-authoring.md) and [cohort](../04-api-design/cohort-scheduling.md) contracts.

Slice 3 adds two narrow dependency-inversion contracts owned with Identity's public account API. `ILearnerCapacityPolicy` lets Licensing inspect only its current verified state through Identity's already-locked transaction; Licensing never mutates accounts. `IInvitationEmailWriter` lets Notifications insert protected work into that transaction; SMTP remains outside it. The host binds both interfaces, and architecture tests prohibit Identity from referencing either implementation assembly. See the concrete [learner provisioning contract](../04-api-design/learner-provisioning.md).
